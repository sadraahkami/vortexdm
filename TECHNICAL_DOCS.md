...
- **Responsive PRO Ribbon Toolbar (3-Tier Adaptive Architecture & Overflow Menu):**
  ...
  - **Sleek Horizontal Scroll & Mouse Wheel Navigation:** When the cumulative width of all 13 buttons with text labels exceeds the available toolbar width, a slim 4px custom cyan glowing scrollbar appears (`.toolbar-buttons::-webkit-scrollbar`). In addition, a custom mouse-wheel listener translates vertical wheel rolls into effortless horizontal scrolling across both Persian RTL and English LTR layouts, preventing any truncation or collision with the speed display panel.
  - **Isolated Speed & Status Section:** `.toolbar-status-right` is shielded with a solid background, z-index elevation, and subtle border separator so scrollable buttons never slide underneath or collide with the speed meter.
